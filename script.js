...
function buildCartHTML() {
  ...
  return '<h2 class="cart-title">Warenkorb</h2>' + listHTML + buildCartFooter();
}

function buildEmptyCart() {
  return `<p class="cart-empty">Dein Warenkorb ist leer.<br>Füge Gerichte hinzu!</p>`;
}
...
function buildCartItem(entry) {
  ...
  return `
    <li class="cart-item">
      ${buildCartItemInfo(dish, quantity)}
      ${buildCartItemControls(dish, quantity)}
    </li>
  `.trim();
}

function buildCartItemInfo(dish, quantity) {
  return `
...
function buildCartItemControls(dish, quantity) {
  return `
    <div class="cart-item-controls">
      <button type="button" class="qty-btn" onclick="updateQuantity(${dish.id}, -1)" aria-label="Weniger">−</button>
      <span class="qty-display">${quantity}</span>
      <button type="button" class="qty-btn" onclick="updateQuantity(${dish.id}, 1)" aria-label="Mehr">+</button>
      <button type="button" class="remove-btn" onclick="removeFromCart(${dish.id})" aria-label="${dish.name} entfernen">✕</button>
    </div>
  `.trim();
}
